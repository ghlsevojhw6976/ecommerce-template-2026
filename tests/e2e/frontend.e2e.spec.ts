...
import { BASE_URL } from '../helpers/testEnv'
import { resetTestFixtures } from '../helpers/resetFixtures'
...
test.describe('Frontend', () => {
  ...
  // This hook drives the admin panel to build products and variants, which
  // means paying for Next's first compile of the admin routes on a cold dev
  // server. That alone can exceed the 30s default, and when it does every test
  // in the file reports as "did not run" with no indication that the cause was
  // simply a slow start.
  ...
  // The homepage headline, title and brand name all come from settings, because
  // this template is deployed as several different shops. Asserting a specific
  // string would mean this test only passes on whichever shop happened to be
  // seeded, so it checks the structure instead: the page renders, and it leads
  // with a non-empty h1.
  ...
  /**
   * POSTs to the REST API and returns the created document.
   *
   * Reading `.doc.id` straight off the response turns every failure — a 403, a
   * validation error, a renamed collection — into "Cannot read properties of
   * undefined (reading 'id')" pointing at line 1 of this file, which says
   * nothing about which call broke or why. This reports the status and body.
   */
  async function createDoc(request: any, collection: string, data: unknown) {
    const response = await request.post(`${baseURL}/api/${collection}`, { data })
    const body = await response.json().catch(() => null)

    if (!response.ok() || !body?.doc) {
      throw new Error(
        `POST /api/${collection} failed (${response.status()}): ${JSON.stringify(body)}`,
      )
    }
  ...
  async function createVariantsAndProducts(page: Page, request: any) {
    const variantTypeID = (
      await createDoc(request, 'variantTypes', { name: 'brand', label: 'Brand' })
    ).id
    ...
    const [payload, figma] = await Promise.all(
      brands.map((option) =>
        createDoc(request, 'variantOptions', { ...option, variantType: variantTypeID }),
      ),
    )

    const payloadVariantID = payload.id
    const figmaVariantID = figma.id

    await loginFromUI(page, adminEmail, adminPassword)
    await page.goto(`${mediaURL}/create`)
    const fileInput = page.locator('input[type="file"]')
    const altInput = page.locator('input[name="alt"]')
    // A committed fixture, NOT a file under public/media. That directory is the
    // uploads dir: it is gitignored, and its contents only exist if the demo
    // seed happens to have run and nothing has cleaned up since. Pointing a
    // test at it makes the suite fail on a fresh clone and on CI.
    const filePath = path.resolve(dirname, '../fixtures/product-image.webp')
    ...
    // Match the id rather than taking the last path segment: the admin appends
    // query parameters after saving, so `.pop()` can hand back "5?locale=en",
    // which the relationship field then rejects as an invalid reference.
    // Numeric because these are Postgres integer ids.
    const imageID = Number(page.url().match(/\/media\/(\d+)/)?.[1])
    expect(Number.isInteger(imageID)).toBe(true)

    const productID = (
      await createDoc(request, 'products', {
    ...
    await createDoc(request, 'variants', {
      product: productID,
