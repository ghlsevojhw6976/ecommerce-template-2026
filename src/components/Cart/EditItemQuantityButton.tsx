...
import { gaItem, trackAddToCart, trackRemoveFromCart } from '@/lib/analytics/gtag'
    ...
    <form>
      <button
        ...
        onClick={(e: React.FormEvent<HTMLButtonElement>) => {
          ...
          if (item.id) {
            // GA4 counts quantity deltas as add/remove of one unit.
            if (typeof item.product === 'object' && item.product && !item.variant) {
              const delta = gaItem(item.product, 1)
              if (type === 'plus') trackAddToCart(delta)
              else trackRemoveFromCart(delta)
            }
            if (type === 'plus') {
