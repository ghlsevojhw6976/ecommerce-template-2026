...
| 2026-08-12 | **Follow-up: the 2026-08-12 warranty sweep above missed the `specifications`/`keyFeatures` array fields entirely** | Owner caught it live: the PDP "Details" table (`Specifications.tsx`, reads `product.specifications`) had a "Warranty Description" / "Manufacturer Warranty Description" / "Warranty Type" row on 133 products, and the "Key features" list had a standalone "Warranty" bullet on 26 more — both populated from `attributes_json`/`new_bullets` at import (`src/lib/import/mapRow.ts`), a completely separate path from the prose `description` the first sweep checked. Confirmed via `grep` before touching anything that `product.specifications` is read in exactly two places in the whole codebase (the importer that writes it, and this one render) — NOT by `mapProduct.ts` (Merchant feed) or the PDP's JSON-LD, so no cross-system dependency to preserve. Fixed three layers: (1) import root cause — added the three `attributes_json` keys to `SPEC_SKIP` and added an `isWarrantyBullet` filter to the `new_bullets` pipeline, so a future catalogue import can't reintroduce this; (2) render-time filter in `Specifications.tsx` (`isWarrantyRow`) as defence in depth, independent of how a row got into the DB; (3) cleaned existing data — deleted 144 `products_specifications` rows and 26 `products_key_features` rows outright (backed up to `/tmp/backup_*_warranty.csv` on the VPS first), and for the 6 rows where warranty was one item in an otherwise-legitimate "Includes"/"Included Components" list, stripped just that list item rather than deleting the row. A `Manufacturer: Lifetime Brands` spec row was checked and left alone — real company name (Lifetime Brands, Inc.), not a warranty claim. `src/scripts/warrantyFieldSweep.ts` is the exhaustive per-field checker (every scalar field, `specifications[]`, `keyFeatures[]`, `inTheBox[]`, reviews) — worth re-running after any future data-hygiene claim rather than trusting a partial grep. |
| 2026-08-12 | **Sitewide `reviewsEnabled` toggle added** (Settings → Company → Policies) | Came out of a request to reinstate the "Verified Purchase" badge removed the day before — declined to just flip that back (it would reintroduce the exact false-purchase-claim problem it was removed for) and asked what the owner actually needed instead; the answer was a full kill switch, not the badge. Off hides the review list AND every rating signal derived from it — star badges on PDP/shop-grid/homepage-hero, and `aggregateRating` in the PDP's own JSON-LD — not just the review list on its own, since a star rating with no reviews behind it is its own inconsistency. Five render sites gated: `Reviews.tsx` self-checks (cheapest gate, checked before the review query even runs); `page.tsx` computes `reviewsEnabled` once and both zeroes the JSON-LD `rating` var and passes an explicit `reviewsEnabled` prop to `ProductDescription` — deliberately a real prop, not inferred from `pooledRating` being falsy, because a falsy `pooledRating` alone still falls through to the product's own `ratingAverage` and would silently defeat the toggle; `ProductGridItem` and `HeroProductCard` (both already server components) became `async` and gate themselves the same way. Reviews are never deleted, only unrendered — the toggle is reversible. The Company global already had `afterChange: [revalidateEverything]`, so saving this field revalidates the whole site for free, no new hook needed. |
...
| 2026-08-14 | **GMC "Unable to do quality & policy checks" — full crawl-path audit; last infra defect fixed (www duplicate host → nginx 301 to apex)** | Live verification as Google sees it: robots.txt correct on prod (all three prior fixes deployed), PDPs 200 to real Googlebot (66.249.\*), feed 200 with correct apex https links/images. nginx access logs showed real Google fetched `/api/media/*` images only twice EVER — both 200, both after the `Allow: /api/media/` fix went live 2026-08-14 — i.e. the images-not-processed backlog is Google re-crawl latency (robots.txt cached ≤24h, image re-crawl days), not a live defect. Remaining real defect found and fixed: nginx served the ENTIRE site on `www.40tag.com` too (one server block, two server_names, no redirect) — the duplicate-host/"two robots.txt" ambiguity Search Console warned about; now a dedicated www 443 block 301s to the apex (see DEPLOYMENT.md §6). ⚠️ Log-reading gotcha: requests claiming `Storebot-Google`/`Googlebot-Image` from non-`66.249.*` IPs (e.g. 92.40.\*) are spoofed UAs, not Google — verify the IP before concluding Google crawled something. |
| 2026-08-19 | **Google Ads conversion tracking admin-managed** (Settings → Analytics: `googleAdsId` AW-… + `googleAdsPurchaseLabel`) — no pasted snippets | Same pattern as GA4: one gtag.js loader serves both products (a `gtag('config')` per configured ID — Google's "install once per account" is literally this); the purchase conversion fires inside `trackPurchaseOnce`'s existing localStorage once-per-order guard with real transaction_id (order id) + charged value/currency, via `window.__adsPurchaseSendTo` baked into the static layout. Consent gating (banner + Consent Mode defaults) now also activates when ONLY the Ads ID is set. Loader works Ads-only, GA-only, or both. |
